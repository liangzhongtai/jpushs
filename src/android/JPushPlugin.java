...
public class JPushPlugin extends CordovaPlugin {
    ...
    public static final int SET_SETTING = 1;
...
//        }
    ...
    static void transmitMessageReceive(String message, Map<String, Object> extras) {
       ...
       JPushUtil.checkSetting(JPushPlugin.instance.mContext,extras);
    }
    ...
    static void transmitNotificationReceive(String title, String alert, Map<String, Object> extras) {
        ...
        cordovaActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Log.d("JPush", "调起js的函数transmitNotificationReceive:" + js);
                instance.webView.loadUrl("javascript:" + js);
            }
        });
    ...
    static void transmitReceiveRegistrationId(String rId) {
        ...
        cordovaActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Log.d("JPush", "调起js的函数transmitReceiveRegistrationId:" + js);
                instance.webView.loadUrl("javascript:" + js);
            }
        });
    }
    ...
    public boolean execute(final String action, final JSONArray data, final CallbackContext callbackContext)
        ...
        Log.d("JPush", "JPush插件调用-----------------" + action);
        // 检查是否已有通知权限
        if ("checkPermission".equals(action)) {
            boolean has = JPushUtil.isPermissionOpen(cordovaActivity.getApplication());
        ...
        // 打开通知权限设置页面
        if ("openSettingPage".equals(action)) {
            JPushUtil.openPermissionSetting(cordovaActivity);
            return true;
        }
        // 打开自启动设置页面
        if ("openAutoStartPage".equals(action)) {
            JPushUtil.openStartInterface(cordovaActivity);
            return true;
        }
        // 获取JPushId
        if ("getJPushId".equals(action)) {
            callbackContext.success(JPushSP.getSPString(cordovaActivity, JPushSP.JPUSH_ID));
            return true;
        }
    ...
    //保存推送设置
    void setting(JSONArray data, CallbackContext callbackContext){
        Context context = JPushPlugin.instance.mContext;
        try {
            jpushType = data.getInt(0);
            if(jpushType==SET_SETTING) {
                JSONObject setting = data.getJSONObject(1);
                JPushSP.setSP(context, JPushSP.USE_PUSH,   setting.getBoolean(JPushSP.USE_PUSH));
                JPushSP.setSP(context, JPushSP.SOUND,      setting.getBoolean(JPushSP.SOUND));
                JPushSP.setSP(context, JPushSP.SHAKE,      setting.getBoolean(JPushSP.SHAKE));
                JPushSP.setSP(context, JPushSP.S_SHOWWEEK, setting.getInt(JPushSP.S_SHOWWEEK));
                JPushSP.setSP(context, JPushSP.E_SHOWWEEK, setting.getInt(JPushSP.E_SHOWWEEK));
                JPushSP.setSP(context, JPushSP.S_SHOWTIME, setting.getInt(JPushSP.S_SHOWTIME));
                JPushSP.setSP(context, JPushSP.E_SHOWTIME, setting.getInt(JPushSP.E_SHOWTIME));
                JPushSP.setSP(context, JPushSP.FREQUENCY,  setting.getInt(JPushSP.FREQUENCY));
            }else if(jpushType==GET_SETTING){
                JSONObject setting = new JSONObject();
                setting.put(JPushSP.USE_PUSH,   JPushSP.getSPBoolean(context,JPushSP.USE_PUSH));
                setting.put(JPushSP.SOUND,      JPushSP.getSPBoolean(context,JPushSP.SOUND));
                setting.put(JPushSP.SHAKE,      JPushSP.getSPBoolean(context,JPushSP.SHAKE));
                setting.put(JPushSP.S_SHOWWEEK, JPushSP.getSPBoolean(context,JPushSP.S_SHOWWEEK));
                setting.put(JPushSP.E_SHOWWEEK, JPushSP.getSPBoolean(context,JPushSP.E_SHOWWEEK));
                setting.put(JPushSP.S_SHOWTIME, JPushSP.getSPBoolean(context,JPushSP.S_SHOWTIME));
                setting.put(JPushSP.E_SHOWTIME, JPushSP.getSPBoolean(context,JPushSP.E_SHOWTIME));
                setting.put(JPushSP.FREQUENCY,  JPushSP.getSPBoolean(context,JPushSP.FREQUENCY));
                PluginResult result = new PluginResult(PluginResult.Status.OK,setting);
